...
class WeMapController: NSObject, FlutterPlatformView, MGLMapViewDelegate, WeMapOptionsSink, MGLAnnotationControllerDelegate {
    ...
    init(withFrame frame: CGRect, viewIdentifier viewId: Int64, arguments args: Any?, registrar: FlutterPluginRegistrar) {  
        ...
        channel = FlutterMethodChannel(name: "plugins.flutter.io/wemap", binaryMessenger: registrar.messenger())
    ...
    func addRasterSource(ID: String, layerURI: String) -> MGLRasterTileSource{
        return MGLRasterTileSource(identifier: ID, tileURLTemplates: [layerURI], options: [ .tileSize: 256 ])
    }

    func addRasterLayer(ID: String, source:MGLSource) -> MGLRasterStyleLayer{
        return MGLRasterStyleLayer(identifier: ID, source: source)
    }
